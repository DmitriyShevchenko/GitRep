...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВидКонтактнойИнформацииАдреса.Вставить("СкрыватьНеактуальныеАдреса", Истина);
	...
	Иначе
		АдресМестаОсуществленияДетельности = УправлениеКонтактнойИнформациейКлиентСервер.ТекстПустогоАдресаВВидеГиперссылки();
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не РегистрацияПоМестуНахожденияОрганизации И ПустаяСтрока(Объект.Адрес) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "АдресМестаОсуществленияДетельности", , Отказ);
	КонецЕсли;
...
&НаКлиенте
Процедура АдресМестаОсуществленияДетельностиНажатие(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ВидКонтактнойИнформации", ВидКонтактнойИнформацииАдреса);
	ПараметрыФормы.Вставить("ЗначенияПолей", Объект.Адрес);
	ПараметрыФормы.Вставить("Представление", ?(Не ПустаяСтрока(Объект.Адрес), АдресМестаОсуществленияДетельности, ""));
	ПараметрыФормы.Вставить("Комментарий", КомментарийАдреса);
	ПараметрыФормы.Вставить("Заголовок", НСтр("ru = 'Адрес места осуществления предпринимательской деятельности'"));
	
	Оповещение = Новый ОписаниеОповещения("АдресМестаОсуществленияДетельностиНажатиеЗавершение", ЭтотОбъект);
...
&НаКлиенте
Процедура АдресМестаОсуществленияДетельностиНажатиеЗавершение(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	
	Если ТипЗнч(Результат) <> Тип("Структура") Тогда
		Возврат;
	КонецЕсли;
	
	Объект.Адрес = Результат.КонтактнаяИнформация;
	КомментарийАдреса = Результат.Комментарий;
	
	Если Не ПустаяСтрока(Объект.Адрес) Тогда
		АдресМестаОсуществленияДетельности = Результат.Представление;
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.АдресМестаОсуществленияДетельности.Видимость = Не Форма.РегистрацияПоМестуНахожденияОрганизации;
	Элементы.КодПоОКТМО.Видимость = Не Форма.РегистрацияПоМестуНахожденияОрганизации;
...
Процедура УстановитьУсловноеОформление()
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "АдресМестаОсуществленияДетельности");
...
Функция СведенияОНалоговомОрганеПоАдресу(Адрес, ВладелецЮрЛицо)
	...
	СведенияОНалоговомОрганеПоАдресу = ДанныеАдресногоКлассификатора.НалоговыйОрганПоАдресу(Адрес);
	Если ЗначениеЗаполнено(СведенияОНалоговомОрганеПоАдресу.ОписаниеОшибки) Тогда
		// Ошибки по работе с сервисом до пользователя не доводим
		Возврат Неопределено;
	КонецЕсли;
	
	Если ВладелецЮрЛицо Тогда
		СведенияОНалоговомОрганеПоАдресу.Вставить("КодНалоговогоОргана", СведенияОНалоговомОрганеПоАдресу.КодНалоговойДляЮридическихЛиц);
	Иначе
		СведенияОНалоговомОрганеПоАдресу.Вставить("КодНалоговогоОргана", СведенияОНалоговомОрганеПоАдресу.КодНалоговойДляФизическихЛиц);
	КонецЕсли;
