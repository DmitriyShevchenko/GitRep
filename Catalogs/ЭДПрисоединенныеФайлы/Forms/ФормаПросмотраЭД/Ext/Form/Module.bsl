...
Процедура ИзменитьВидимостьДоступность()
	...
	ЭДТитулПродавца	= Объект.ВидЭД = Перечисления.ВидыЭД.УПД_ИнформацияПродавца
	...
	ЭДСчетФактура	= ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоСчетФактура(Объект.Ссылка);
	...
	Если Не ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ЗначениеФункциональнойОпции("ИспользоватьЭлектронныеПодписиЭД")
		И ЭлектронноеВзаимодействиеСлужебныйВызовСервера.НемедленнаяОтправкаЭД() Тогда
		Элементы.КомандаУтвердить.Заголовок = НСтр("ru = 'Утвердить и отправить'");
	КонецЕсли;
	
	Если ЭлектронноеВзаимодействиеСлужебныйВызовСервера.НемедленнаяОтправкаЭД() Тогда
	...
	Если Объект.НаправлениеЭД = Перечисления.НаправленияЭД.Входящий Тогда
		...
		Если Объект.ВидЭД = Перечисления.ВидыЭД.УКД_ИнформацияПокупателя
			ИЛИ Объект.ВидЭД = Перечисления.ВидыЭД.УПД_ИнформацияПокупателя
	...
	ИначеЕсли Объект.НаправлениеЭД = Перечисления.НаправленияЭД.Исходящий Тогда
		...
		КомандаПодписиОтправки.Доступность = НЕ СтатусОтклонен И Не ЭДОЗакрыт
			И (ОтраженВУчете ИЛИ Объект.ВидЭД = Перечисления.ВидыЭД.УКД_ИнформацияПокупателя
				ИЛИ Объект.ВидЭД = Перечисления.ВидыЭД.УПД_ИнформацияПокупателя
				ИЛИ Объект.ВидЭД = Перечисления.ВидыЭД.ТОРГ12Покупатель
				ИЛИ Объект.ВидЭД = Перечисления.ВидыЭД.АктЗаказчик
...
Функция ЭлектронныеПодписиДвухТитулов()
	...
	ЭПРезультат = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "ЭлектронныеПодписи");
...
Процедура ОпределитьНаличиеДопДанныхСкрытьФлаг()
	...
	Если СсылкаНаЭД.ВидЭД = Перечисления.ВидыЭД.УКД_ИнформацияПродавца
		ИЛИ СсылкаНаЭД.ВидЭД = Перечисления.ВидыЭД.УКД_ИнформацияПокупателя
		ИЛИ СсылкаНаЭД.ВидЭД = Перечисления.ВидыЭД.УПД_ИнформацияПродавца
		ИЛИ СсылкаНаЭД.ВидЭД = Перечисления.ВидыЭД.УПД_ИнформацияПокупателя
		...
		ИЛИ СсылкаНаЭД.ВидЭД = Перечисления.ВидыЭД.ОтчетОСписанииКомиссионногоТовара Тогда
...
Функция ВторойТитулПодписан()
	...
	Результат = Ложь;
	ФайлВторогоТитула = ОбменСКонтрагентамиСлужебный.ВторойТитулДокумента(Объект.Ссылка);
