...
&НаКлиенте
Процедура СостояниеТорговойТочкиИстория(Команда)
...
&НаКлиенте
Процедура ПлощадьТорговогоЗалаИстория(Команда)
	
	ПараметрыФормы = ПараметрыФормыИсторияПараметров();
	ОписаниеОповещения = Новый ОписаниеОповещения("ПлощадьТорговогоЗалаИсторияЗавершение", ЭтотОбъект);
...
&НаКлиенте
Процедура НалоговаяЛьготаИстория(Команда)
	
	ПараметрыФормы = ПараметрыФормыИсторияПараметров();
	ОписаниеОповещения = Новый ОписаниеОповещения("НалоговаяЛьготаИсторияЗавершение", ЭтотОбъект);
...
&НаКлиенте
Процедура ПлощадьТорговогоЗалаИсторияЗавершение(ИсторияИзменена, ДопПараметры) Экспорт
...
&НаКлиенте
Процедура НалоговаяЛьготаИсторияЗавершение(ИсторияИзменена, ДопПараметры) Экспорт
...
Процедура УстановитьВидимостьЭлементов(ЭтотОбъект)
	...
	Параметры = ЭтотОбъект.ПараметрыТорговыхТочек;
	...
	Если НЕ ЭтотОбъект.ВКонфигурациюВстроенаБРО Тогда
		...
		Элементы.НадписьУведомлениеПодано.Видимость = Истина;
	...
	Элементы.ГруппаДатаПостановкиНаУчет.Видимость     = ЭтоНоваяЗапись;
	Элементы.ГруппаСостояниеТорговойТочки.Видимость   = НЕ ЭтоНоваяЗапись;
	...
	Элементы.ПлощадьТорговогоЗалаИстория.Видимость    = НЕ ЭтоНоваяЗапись;
	Элементы.НалоговаяЛьготаИстория.Видимость         = НЕ ЭтоНоваяЗапись;
...
Процедура УстановитьПредставлениеИзменяемыхПараметров()
	...
	ПлощадьТорговогоЗалаНадпись = Новый Массив;
	ПлощадьТорговогоЗалаНадпись.Добавить(ПараметрыТорговыхТочек.ПлощадьТорговогоЗала);
	ПлощадьТорговогоЗалаНадпись.Добавить(НСтр("ru='кв.м.'"));
	Элементы.ПлощадьТорговогоЗалаНадпись.Заголовок = СтрСоединить(ПлощадьТорговогоЗалаНадпись, " ");
	
	Элементы.НалоговаяЛьготаНадпись.Заголовок =
		РегистрыСведений.ПараметрыТорговыхТочек.НаименованиеПоКодуЛьготы(
...
Процедура УстановитьТекстСостоянияТорговойТочки()
	...
	Элементы.СостояниеТорговойТочкиИстория.Заголовок = СтрСоединить(Результат, " ");
...
Процедура СохранитьОКТМО()
	...
	Попытка
		СведенияОбАдресе = УправлениеКонтактнойИнформацией.СведенияОбАдресе(НовыйАдрес);
		Если ЗначениеЗаполнено(СведенияОбАдресе.ДополнительныеКоды.ОКТМО) Тогда
			ПараметрыТорговыхТочек.КодПоОКТМО = СтрЗаменить(СведенияОбАдресе.ДополнительныеКоды.ОКТМО, Символы.НПП, "");
		КонецЕсли;
		
	Исключение
		// не требуется обработки
	КонецПопытки;
...
Процедура РазместитьКонтактнуюИнформацию()
	...
	УправлениеКонтактнойИнформацией.ПриСозданииНаСервере(ЭтотОбъект, Объект, "ГруппаКонтактнаяИнформация",
		ПоложениеЗаголовкаЭлементаФормы.Лево, ИсключаемыеВиды);
...
Процедура ЗаполнитьАдресПоОбособленномуПодразделению(Подразделение)
	...
	КИПодразделения = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(
		МассивОбъектов,,
		Справочники.ВидыКонтактнойИнформации.ФактическийАдресПодразделенияОрганизаций);
	...
	СтрокаКИ.ЗначенияПолей = КИПодразделения[0].ЗначенияПолей;
