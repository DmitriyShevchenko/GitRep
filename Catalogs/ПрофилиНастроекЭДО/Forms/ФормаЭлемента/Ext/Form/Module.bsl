	...
	Иначе
		...
		Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
			...
			Если ТипВыбранногоЗначения = Тип("СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования") Тогда
				...
				Если ОбменСКонтрагентамиСлужебныйКлиент.ПроверитьИспользованиеИнтернетПоддержкаПользователей()
...
Процедура ИсходящиеДокументыВерсияФорматаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.ТОРГ12Продавец")
		ИЛИ ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.АктИсполнитель") Тогда
		СписокВыбора.Добавить("ФНС 5.01", "ФНС 5.01 (до 31.12.2016г.)");
		СписокВыбора.Добавить("ФНС 5.01 (с 2016г.)", "ФНС 5.01 (с 01.01.2016г.)");
	Иначе
...
Процедура ПерейтиВЛичныйКабинетОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(СертификатПодписи) Тогда
		ОбменСКонтрагентамиКлиентПереопределяемый.СтартоватьМеханизмРаботыСОператоромЭДО(СертификатПодписи,
		Объект.Организация,
		"taxcomPrivat",
...
Процедура ЗарегистрироватьСертификатТакском(СертификатЭП)
	...
	ОбменСКонтрагентамиСлужебныйКлиент.ТестНастроекСПроверкойСертификата(СертификатЭП,
...
Процедура ЗарегистрироватьСертификатТакскомОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Истина Тогда
		...
		ОбменСКонтрагентамиСлужебныйКлиент.ПарольКСертификатуПолучен(СертификатЭП, Параметр);
		ОбменСКонтрагентамиКлиентПереопределяемый.СтартоватьМеханизмРаботыСОператоромЭДО(СертификатЭП,
																						  Объект.Организация,
																						  "taxcomGetID",
																						  Объект.ИдентификаторОрганизации,
																						  Параметр,
...
Процедура НастройкаУведомленийЗакрытие(ПараметрыОповещения, ДополнительныеПараметры) Экспорт
	...
	Если Не ПараметрыОповещения.АдресУведомлений = Объект.ЭлектроннаяПочтаДляУведомлений Тогда
	...
	Если Не ПараметрыОповещения.УведомлятьОСобытиях = УведомлятьОСобытиях Тогда
