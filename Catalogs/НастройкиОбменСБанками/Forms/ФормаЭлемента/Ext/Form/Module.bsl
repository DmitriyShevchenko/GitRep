...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Объект.ПрограммаБанка = Перечисления.ПрограммыБанка.СбербанкОнлайн Тогда
		...
		Исключение
			...
			Операция = НСтр("ru = 'Открытие настройки DirectBank'");
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если СохранитьВнешнююКомпонентуСбербанка ИЛИ ТребуетсяИнициализацияВнешнегоМодуля Тогда
	...
	Если СохранитьВнешнийМодуль Тогда
		...
		Если ПараметрыОбменаСБанками <> Неопределено Тогда
			ВнешниеКомпоненты = ПараметрыОбменаСБанками.Получить("ВнешниеКомпоненты");
			Если ВнешниеКомпоненты <> Неопределено Тогда
				ВнешниеКомпоненты.Удалить(Объект.ИмяВнешнегоМодуля);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗагрузитьВнешнийМодульИзФайлаНажатие(Элемент)
	...
	#Иначе
		...
		ДиалогОткрытияФайла.Заголовок = "Выберите файл внешнего модуля банка";
...
Процедура ИнформацияОКомпонентеСбербанкаНажатие(Элемент)
	...
	#Иначе
		...
		Обработчик = Новый ОписаниеОповещения("СохранитьКомпонентуПослеВыбораФайла", ЭтотОбъект);
		ДиалогОткрытияФайла.Показать(Обработчик);
	#КонецЕсли
...
Процедура СертификатыПодписейОрганизацииОбработкаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	НовыйСертификат = НайтиСоздатьСертификатВнешнегоМодуля(
		ВыбранноеЗначение, Объект.Организация, Объект.ПрограммаБанка, ДанныеСертификата);
...
&НаКлиенте
Процедура ПослеИзмененияНастроекЖурналирования(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.ИспользоватьЖурналирование <> Объект.ИспользоватьЖурналирование
		ИЛИ Результат.КаталогДляЖурналирования <> Объект.КаталогДляЖурналирования Тогда
		ЗаполнитьЗначенияСвойств(Объект, Результат);
		ТребуетсяИнициализацияВнешнегоМодуля = Истина;
	КонецЕсли;
...
&НаКлиенте
Процедура ЗагрузитьСертификатЧерезВК() Экспорт
...
&НаКлиенте
Процедура СохранитьКомпонентуПослеВыбораФайла(МассивФайлов, ДополнительныеПараметры) Экспорт
...
Процедура ПродолжитьПолучениеСертификата(ПараметрыПолучения)
	...
	ПараметрыОткрытия = Новый Структура("НастройкаОбмена, ИдентификаторХранилища", Объект.Ссылка, ИдентификаторХранилища);
	ОткрытьФорму("Обработка.ОбменСБанками.Форма.ПолучениеСертификата", ПараметрыОткрытия, ЭлементОповещения);
...
Процедура ОбработатьВыборФайлаВнешнегоМодуля(ИмяФайлаВнешнегоМодуля)
	...
	Иначе // внешняя обработка
		...
		#Иначе
			...
			ОбменСБанкамиСлужебныйВызовСервера.ПодключитьВнешнююОбработку(АдресВнешнегоМодуля, ИмяВнешнегоМодуля);
...
Процедура ПослеПолученияВерсииКомпонентыСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Строка") И Результат <> Объект.ВерсияВнешнейКомпоненты Тогда
		#Если ВебКлиент Тогда
			ТекстСообщения = Нстр("ru = 'На компьтере уже установлен внешний модуль другой версии.
...
&НаКлиенте
Процедура ПослеВыбораВерсииДополнительнойОбработки(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = ДополнительныеПараметры.ВерсияВнешнегоМодуля Тогда // Заменить внешнюю обработку
		Модифицированность = Истина;
		СохранитьВнешнийМодуль = Истина;
		Шаблон = НСтр("ru = '%1. Версия %2.'");
		Элементы.ИнформацияОВнешнемМодуле.Заголовок = СтрШаблон(
			Шаблон, ДополнительныеПараметры.НаименованиеВнешнегоМодуля, ДополнительныеПараметры.ВерсияВнешнегоМодуля);
	Иначе
		СохранитьВнешнийМодуль = Ложь;
		ЗаполнитьИнформациюОВнешнемМодуле();
	КонецЕсли;
