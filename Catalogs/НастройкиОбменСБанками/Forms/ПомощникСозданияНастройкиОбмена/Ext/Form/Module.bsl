...
&НаКлиенте
Перем АдресФайлаВнешнегоМодуля; // адрес временного хранилища внешней компоненты банка
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НаименованиеЗадания = Нстр("ru = 'DirectBank: Обновление списка банков, поддерживающих обмен.'");
	...
	Если НЕ НеПоказыватьСтраницуИнформации Тогда
		Описание = Справочники.НастройкиОбменСБанками.ПолучитьМакет("Описание").ПолучитьТекст();
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗакрытьФорму И НЕ Элементы.КомандыНавигации.ТекущаяСтраница = Элементы.Отмена
		И (Элементы.Шаги.ТекущаяСтраница <> Элементы.СтраницаФинал // не достигнут последний шаг
			ИЛИ Элементы.СтраницыРезультатПодключения.ТекущаяСтраница <> Элементы.СтраницаПодключено) Тогда // произошла ошибка
...
Процедура Далее(Команда)
	...
	ИначеЕсли Элементы.Шаги.ТекущаяСтраница = Элементы.СтраницаВыборСпособаСозданияНастройки Тогда
		...
		ИначеЕсли СпособСозданияНастройки = 2 Тогда // ручной режим
			...
			Иначе
				Если РазделениеВключено Тогда
					МассивВКВИБ = ВнешниеКомпонентыИнформационнойБазы();
					Если МассивВКВИБ.Количество() = 0 Тогда
						СменитьСтраницуШаги(Элементы.СтраницаАсинхронныйОбмен);
					Иначе
						Для Каждого Элемент Из МассивВКВИБ Цикл
							Элементы.ИмяВнешнегоМодуля.СписокВыбора.Добавить(Элемент.ИмяМодуля, Элемент.Наименование);
						КонецЦикла;
						Если МассивВКВИБ.Количество() = 1 Тогда
							ИмяВнешнегоМодуля = Элемент.ИмяМодуля;
						КонецЕсли;
...
Процедура СменитьСтраницуШаги(НоваяСтраница)
	...
	ИначеЕсли НоваяСтраница = Элементы.СтраницаВыборСпособаСозданияНастройки Тогда
		Если ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.СбербанкОнлайн") Тогда
			...
			КонецЕсли
		Иначе
			Элементы.ПолучениеНастроекССервераБанка.Видимость = Истина;
		КонецЕсли;
...
Процедура ПереключитьШагПослеВыбораОрганизацииИБанка()
	...
	Элементы.РаспоркаСмешанногоРежима.Видимость = Истина;
	Элементы.РаспоркаСмешанногоРежима.Ширина = 14;
	...
	ИначеЕсли НастройкиОбменаСБанком.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.АсинхронныйОбмен") Тогда
		...
		Если ЗначениеЗаполнено(НастройкиОбменаСБанком.СпособАутентификации) Тогда
			...
			ИначеЕсли НастройкиОбменаСБанком.СпособАутентификации = "ПоСертификату" Тогда
				...
				Элементы.РаспоркаСмешанногоРежима.Видимость = ТестовыйРежим;
				Элементы.РаспоркаСмешанногоРежима.Ширина = 9;
				Оповещение = Новый ОписаниеОповещения(
...
Процедура ОбработатьРезультатПолученияИзвещенияОСостоянииЭД(МассивСообщений)
	...
	Оповестить("ОбновитьСостояниеЭД", СтруктураВозврата.ПараметрОповещения);
...
Процедура ЗагрузитьНастройкуОбменаИзФайла()
	...
	Задание = ЗагрузитьНастройкуОбменаИзФайлаНаСервере(АдресФайлаНастроекОбмена, УникальныйИдентификатор, ЗагружатьВК);
...
Функция ЗагрузитьНастройкуОбменаИзФайлаНаСервере(Знач АдресФайлаНастроекОбмена, Знач ИдентификаторФормы, Знач ЗагружатьВК)
...
Процедура ПолучитьИдентификаторСессииПослеПолученияИнформацииОМенеджереКриптографии(ИнформацияМодуля, ДополнительныеПараметры) Экспорт
	...
	ИдентификаторСессии = ОбменСБанкамиСлужебныйВызовСервера.МаркерБанкаПоСертификату(
		АдресСервераНастроек, "0", ДополнительныеПараметры.ДанныеСертификата, , СпособСозданияНастройки = 2);
...
Процедура ПолучитьНастройкиОбменаПослеПолученияИдентификатораСессии(ИдентификаторСессии, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Задание = ЗагрузитьНастройкиОбменаНаСервере(АдресСервераНастроек, Организация, Банк, ИдентификаторСессии,
		УникальныйИдентификатор, ПробнаяОперация, АдресФайлаНастроекОбмена);
...
&НаСервереБезКонтекста
Функция ЗагрузитьНастройкиОбменаНаСервере(Знач АдресСервераНастроек, Знач Организация, Знач Банк, Знач ИдентификаторСессии, Знач УникальныйИдентификатор, Знач ПробнаяОперация, АдресФайлаНастроекОбмена)
...
Процедура ПослеВыбораСертификатаЧерезДополнительнуюОбработку(ДанныеВыбора, ДополнительныеПараметры) Экспорт
	...
	ПарольУстановлен = ОбменСБанкамиСлужебныйКлиент.УстановитьПарольСертификатаЧерезДополнительнуюОбработку(
		ПодключаемыйМодуль, СертификатXML, ДанныеВыбора.Пароль, УникальныйИдентификатор);
...
&НаКлиенте
Процедура ВыбратьВерсиюИспользуемогоВнешнегоМодуля(Оповещение, Версия1, Версия2) Экспорт
...
Процедура ПослеУстановкиВнешнейКомпоненты(ДополнительныеПараметры) Экспорт
	...
	НачатьПодключениеВнешнейКомпоненты(Оповещение, АдресФайлаВнешнегоМодуля, ИдентификаторВК, ТипВнешнейКомпоненты.Native);
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеПолученияСертификатовВК(Результат, ДополнительныеПараметры) Экспорт
	...
	ДополнительныеПараметры.Вставить("ДанныеСертификатовНаТокене", Результат);
...
&НаКлиенте
Процедура ПолучитьСертификатыСТокенаПослеПодключенияВК(Результат, ДополнительныеПараметры) Экспорт
	...
	Оповещение = Новый ОписаниеОповещения("ПослеПолученияСертификатовВК", ЭтотОбъект, ДополнительныеПараметры);
	
	ОбменСБанкамиСлужебныйКлиент.ПолучитьСертификатыИзХранилищаВК(Оповещение, ПодключаемыйМодуль);
...
&НаКлиенте
Процедура СоздатьОбъектПодключаемогоМодуля() Экспорт
...
Функция ДобавитьСертификатВНастройкуОбменаВК(Знач НастройкаОбмена, Знач СертификатBase64, Знач СтруктураСертификата)
	...
	Попытка
		...
		НастройкаОбменаОбъект.Заблокировать();
	...
	Исключение
		Операция = Нстр("ru = 'Добавление сертификата в настройку DirectBank'");
		ШаблонСообщения = НСтр("ru = 'При добавлении сертификата в настройку DirectBank произошла ошибка.
									|%1'");
...
&НаСервереБезКонтекста
Функция ВнешниеКомпонентыИнформационнойБазы()
	
	Возврат ОбменСБанкамиСлужебныйПовтИсп.ВнешниеКомпонентыИнформационнойБазы();
...
Процедура ПолучитьИВывестиИнформациюОбОшибке(ВидОперации, ТекстСообщения)
	...
	ШаблонОшибки = ТекстСообщения + Символы.ПС + НСтр("ru = 'Код ошибки: ДО-%1
															|%2'");
	ПодробныйТекстОшибки = СтрШаблон(ШаблонОшибки, Формат(ДеталиОшибки.Код, "ЧН=0; ЧГ="), ДеталиОшибки.Сообщение);
	ОбменСБанкамиСлужебныйКлиент.СообщитьПользователю(ТекстСообщения, УникальныйИдентификатор);
	ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(ВидОперации, ПодробныйТекстОшибки, , 1);
...
&НаКлиенте
Процедура ОбработатьРезультатПолученияНастроекССервераБанка(Успех) Экспорт
...
&НаКлиенте
Процедура ОбработатьРезультатСозданияНастройкиОбмена(Успех) Экспорт
...
&НаКлиенте
Процедура ОбработатьРезультатТестаНастройки(Успех) Экспорт
...
Процедура ВключитьНастройкуОбмена(Знач НастройкаОбмена, Успех)
	...
	Исключение
		...
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВидОперации = Нстр("ru = 'Включение настройки DirectBank'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
